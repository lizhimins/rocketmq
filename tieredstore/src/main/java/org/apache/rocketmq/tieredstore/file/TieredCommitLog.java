        ...
        try {
            return this.flatFile.readAsync(this.flatFile.getMinOffset(), length)
                .thenApply(buffer -> {
                    ...
                    log.info("Correct min offset in tiered commitlog success, offset={}, filePath={}, range={}-{}",
                        offset, flatFile.getFilePath(), flatFile.getMinOffset(), flatFile.getCommitOffset());
                    return offset;
                })
                .exceptionally(throwable -> {
                    log.warn("Correct min offset in tiered commitlog error, filePath={}, range={}-{}",
                        flatFile.getFilePath(), flatFile.getMinOffset(), flatFile.getCommitOffset(), throwable);
                    return minConsumeQueueOffset.get();
                }).get();
        } catch (Exception e) {
            log.error("Correct min offset in tiered commitlog error, filePath={}", flatFile.getFilePath(), e);
        }
        return minConsumeQueueOffset.get();
    }
