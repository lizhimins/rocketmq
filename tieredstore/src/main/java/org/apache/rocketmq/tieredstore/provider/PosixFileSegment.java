...
import static org.apache.rocketmq.tieredstore.metrics.MessageStoreMetricsConstant.LABEL_PATH;
import static org.apache.rocketmq.tieredstore.metrics.MessageStoreMetricsConstant.LABEL_SUCCESS;
...
public class PosixFileSegment extends FileSegment {
    ...
    protected AttributesBuilder newAttributesBuilder() {
        return MessageStoreMetricsManager.newAttributesBuilder()
    ...
    public CompletableFuture<ByteBuffer> read0(long position, int length) {
        ...
        try {
            ...
            MessageStoreMetricsManager.downloadBytes.record(downloadedBytes, metricsAttributes);
    ...
    public CompletableFuture<Boolean> commit0(
        ...
        MessageStoreExecutor storeExecutor = new MessageStoreExecutor(100);
        CompletableFuture<Boolean> future = new CompletableFuture<>();
        try {
            storeExecutor.bufferCommitExecutor.execute(() -> {
                try {
                    ...
                    Attributes metricsAttributes = newAttributesBuilder()
                        .put(LABEL_OPERATION, OPERATION_POSIX_WRITE)
                        .build();
                    MessageStoreMetricsManager.uploadBytes.record(length, metricsAttributes);
