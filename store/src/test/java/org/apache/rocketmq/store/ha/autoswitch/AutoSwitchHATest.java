...
public class AutoSwitchHATest {
    ...
    private String getHaAddress(DefaultMessageStore messageStore) {
    ...
    public void testTransferMessage() throws Exception {
        ...
        messageStore2.getHaService().updateHaMasterAddress(getHaAddress(messageStore1));
    ...
    private void changeMasterAndPutMessage(int epoch, DefaultMessageStore masterStore,
        DefaultMessageStore slaveStore, long slaveId, int totalPutMessageNums) throws Exception {

        String masterHaAddress = masterStore.getMessageStoreConfig().getHaMasterAddress();
    ...
    public void testOptionAllAckInSyncStateSet() throws Exception {
        initMessageStore(DEFAULT_MAPPED_FILE_SIZE);
        this.messageStore1.getMessageStoreConfig().setHaMasterAddress("127.0.0.1:7000");

        int messageCount = 10;
        changeMasterAndPutMessage(1, this.messageStore1, this.messageStore2, 1, messageCount);
        await().pollInterval(Duration.ofSeconds(1)).atMost(Duration.ofSeconds(30)).until(
            () -> messageCount == getMessageCount(messageStore2));
        ...
        // Reset in sync state set to both store1 and store2
