...
public class MoveAndDiscardTruncateStrategy implements TruncateStrategy {
    ...
    /**
     *  Finish dispatching the messages fall behind, then to start other services.
     */
    @Override
    public long truncateInvalidMsg(DefaultMessageStore defaultMessageStore) {
        ...
        while (reputFromOffset < defaultMessageStore.getMaxPhyOffset() && doNext) {
            ...
            try {
                ...
                while (readSize < result.getSize()) {
                    DispatchRequest dispatchRequest = defaultMessageStore.getCommitLog().checkMessageAndReturnSize(result.getByteBuffer(), false, false);
